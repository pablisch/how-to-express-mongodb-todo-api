...
describe('POST /todos', () => {
  test.each(['Climb', 'Swim', 'Climb a tree'])(
    'should add a todo to the database and return status 201 and an array containing the created todo object',
    async (task) => {
      // Act
...
test.each([
  ...
  'should return status 400 and an appropriate error message when given task value of %s',
...
describe('deleteTodo()', () => {
  test.each([
    ...
    'should delete todo with id %s from the database and return status 200 and an array containing only the deleted todo object',
...
describe('DELETE /todos/:id', () => {
  test.each([
    ...
    'should delete todo from database and return status 200 and an array with the deleted todo object with id, "%s"',
    async (id, task, completed) => {
      // Act
...
test.each([
  ...
  'should return an appropriate status and error message when passed the ID param, "%s"',
  async (id, status, errorMessage) => {
    // Act
    const response = await request(app).delete(`/todos/${id}`)
...
describe('updatePutTodo()', () => {
  test.each([
    ...
    'should update todo in the database and return status 201 and the updated todo object',
    async (id, task, completed) => {
      // Arrange
      const mReq = {
        params: {
...
if (!task || (!completed && completed !== false))
  return next({
    status: 400,
    message: 'Task and completed properties are required to update this todo.',
  })
```
...
if (typeof completed !== 'boolean')
  return next({
    status: 400,
    message: 'The completed property must be of type boolean.',
  })
if (typeof task !== 'string')
  return next({
    status: 400,
    message: 'The task property must be of type string.',
  })
```
...
test.each([
  [
    ...
    false,
    400,
    'Task and completed properties are required to update this todo.',
  ],
  [
    2,
    'Dream',
  ...
  [
    ...
    true,
    404,
    'No todo with an ID of 3000 could be found in the database.',
  ],
  [
    0,
    'Put',
...
describe('UPDATE PUT /todos/:id', () => {
  test.each([
    ...
    'should update todo in the database and return status 201 and an array with the updated todo object with id, "%s"',
    async (id, task, completed) => {
      // Act
...
if (!task && !completed && completed !== false)
  return next({
    status: 400,
    message:
      'Either task or completed property are required to update this todo. Neither was provided.',
  })
```
...
if (completed && typeof completed !== 'boolean')
  return next({
    status: 400,
    message: 'If provided, the completed property must be of type boolean.',
  })
if (task && typeof task !== 'string')
  return next({
    status: 400,
    message: 'If provided, the task property must be of type string.',
  })
```
...
test.each([
  ...
  [
    ...
    'hello',
    400,
    'If provided, the completed property must be of type boolean.',
  ],
  ...
  [
    ...
    true,
    400,
    'If provided, the task property must be of type string.',
  ],
  [
    1,
    'Patch',
...
describe('UPDATE PATCH /todos/:id', () => {
  test.each([
    ...
    'should update todo in the database and return status 201 and an array with a completely updated todo with when id = %s, task = %s and completed = %s',
    async (id, task, completed) => {
      // Act
      const response = await request(app)
        .patch(`/todos/${id}`)
  ...
  test.each([
    ...
    'should update todo in the database and return status 201 and an array with a partially updated todo when id = %s, task = %s and completed = %s',
    async (id, task, completed) => {
      // Act
      const response = await request(app)
        .patch(`/todos/${id}`)
...
test.each([
  ...
  [
    ...
    false,
    404,
    'No todo with an ID of 30 could be found in the database.',
  ],
  [
    0,
    'Patch',
  ...
  'should return an appropriate status and error message when id = %s, task = %s and completed = %s',
  async (id, task, completed, status, errorMessage) => {
    // Act
    const response = await request(app)
      .patch(`/todos/${id}`)
